...
for (const user of users) {
  for (const favoritesSound in user.favoritesSounds) {
    soundsArray.push(favoritesSound);
  }
}

let soundsCounter = [];

for (let index = 0; index < soundsArray.length; index++) {
 if (soundsCounter[soundsArray[index]]) {
  soundsCounter[soundsArray[index]]++;
 }
else{
  soundsCounter[soundsArray[index]] = 1;
}}
